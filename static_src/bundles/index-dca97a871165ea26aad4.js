...
const testGal = document.getElementsByClassName('btnTest');

// document.getElementsByClassName(oneGal).classList.toggle('test');

testGal.map(addEventListener('click' , function(e) {
  console.log(filter(e.target.dataset.cat));
  for(var i=0, len=oneGal.length; i<len; i++)
  {
...
function filter(cat) {
  if (cat in cats ) {
    cats.fruits.splice(pos, cats.indexOf(cat));
  } else {
    cats.push(cat);
  }
  return cats ;
